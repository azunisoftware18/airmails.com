...
export const createRazorpayOrder = (orderData) => async (dispatch) => {
  ...
  try {
    ...
    dispatch(
      subscriptionFail(err?.response?.data?.message || "Order creation failed")
    );
    throw err;
  }
};
...
export const createOrRenewSubscriptionAction =
...
export const getMySubscription = () => async (dispatch) => {
  try {
    ...
    dispatch(
      subscriptionFail(
        err?.response?.data?.message || "Fetching subscription failed"
      )
    );
    throw err;
  }
...
export const cancelSubscription = () => async (dispatch) => {
  try {
    ...
    dispatch(
      subscriptionFail(
        err?.response?.data?.message || "Cancel subscription failed"
      )
    );
    throw err;
  }
